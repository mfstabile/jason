...
public class TransitionSystem implements Serializable {
    ...
    private boolean anytimeProfiling = true;
    ...
    private void applyExecInt() throws JasonException {
        ...
        case action:
            ...
            if (agArch instanceof CentralisedAgArchAnytimeAsynchronous) {
                ...
                stepAct = State.StartRC;
//                logger.severe("Yes-----Executing action " + h + "<->" + body );
            }else {
//                LOG.info("WRONG_-_-_Executing action " + h + "<->" + body );
        ...
        case test:
            ...
            if (ag.believes(f, u)) {
                ...
                while(C.isUpdatingBeliefs){
                    if (ag.believes(f, u)) {
                        removeActionReQueue(curInt);
                        beliefFound = true;
                    }
                }
                if(beliefFound==false){
                    boolean fail = true;
    ...
    public void anytimeSense() {
        try {
            do {
                synchronized (C.syncApPlanSense) {
                    C.isUpdatingBeliefs = true;
                    ag.anytimeBUF(getAgArch().perceive());
                    C.isUpdatingBeliefs = false;
                }
                getAgArch().checkMail();
